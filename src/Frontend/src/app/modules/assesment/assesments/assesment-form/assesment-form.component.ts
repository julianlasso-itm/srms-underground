...
const URL_GET_ASSESSMENT = `${Constant.URL_BASE}${Constant.URL_GET_ASSESSMENT}`;
...
export class AssesmentFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.assesment()?.assesmentId !== undefined) {
      ...
      this.frmAssesment()
        .get('squadId')
        ?.setValue(this.assesment()?.assesmentId);
      this.frmAssesment()
        .get('roleId')
        ?.setValue(this.assesment()?.assesmentId);
      this.frmAssesment()
  ...
  private _filter(value: string | IAssesment, options: any[]): any[] {
    ...
    return options.filter((option) =>
      option.name.toLowerCase().includes(value.name)
    );
  }
  ...
  private createAssesment(): void {
    ...
    this.httpService.post(URL_GET_ASSESSMENT, body).subscribe({
      next: (response) => {
  ...
  private updateAssesment(): void {
    ...
    const url = `${URL_GET_ASSESSMENT}/${this.assesment()?.assesmentId}`;
