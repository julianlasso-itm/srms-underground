...
export class ProfessionalFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.professional()?.professionalId !== undefined) {
      ...
      this.frmProfessional().get('name')?.setValue(this.professional()?.name);
      this.frmProfessional().get('email')?.setValue(this.professional()?.email);
      this.frmProfessional().setControl(
        'disabled',
        new FormControl<boolean>(!this.professional()?.disabled || false)
      );
    }
  ...
  private updateProfessional(): void {
    ...
    const url = `${URL_PROFESSIONAL}/${this.professional()?.professionalId}`;
